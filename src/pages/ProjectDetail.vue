...
<script>
  export default {
    ...
    data() {
      ...
      const selectedCompany = projectCollections.find((item) => item.name?.replace(/[' ']*/g, '')?.toLowerCase() === id.toLowerCase())
      const tabs = [
        'Overview',
        'What I Do',
        'Timeline',
...
<template>
  <div class="w-11/12 mx-auto pt-10 flex flex-row gap-10">
    <div class="bg-black/10 p-10 w-5/12 h-[400px] flex flex-col justify-center">
      <img class="self-center w-1/2 p-2 sm:p-3" :src="`/assets/images/logo/${project?.image}`" :alt="project?.name || 'img'" />
    </div>
